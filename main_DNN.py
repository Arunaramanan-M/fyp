...
np.random.seed(4873256)  # To get the same UE distribution every time you try something new
    ...
    for l in range(0, L):
        ...
        for t in range(0, tau_p):
            ...
            for k in  np.argwhere(t==pilotIndex):
                ...
                Hhat_MMSE_MeanSquare[int(k),l] =  (p * tau_p/ M) * np.real(np.trace (  np.matmul( RPsi, CorrR[:,:,int(k),l] ) )  )
...
plt.plot(sorted_SE_MR_Giovanni19, Yvals, '-.', color='#000000ff', label='Heuristic method', linewidth=4)
...
plt.plot(sorted_SE_RZF_Giovanni19, Yvals, '-.', color='#000000ff', label='Heuristic method', linewidth=4)   
...
plt.plot(sum_SE_MR_Giovanni19, Yvals_sum, '-.', color='#000000ff', label='Heuristic method', linewidth=4)   
...
plt.plot(sum_SE_RZF_Giovanni19, Yvals_sum, '-.', color='#000000ff', label='Heuristic method', linewidth=4)   
